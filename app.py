...
# 1. إعداد الصفحة (يجب أن يكون أول شيء)
st.set_page_config(page_title="المنصة التعليمية قِصَّتِي دِرَاسَتِي", page_icon="🎈", layout="wide")

# 2. تهيئة الحالة (يجب أن توضع هنا قبل أي استخدام لـ session_state)
    ...
    if data:
        ...
        if st.button("تحقق من إجابتي!", key=f"btn_check_{اسم_الدرس}_{مستوى_السنة}"):
            if choix == data['إجابة']:
                ...
                st.success(f"🎉 إجابة صحيحة! نقاط المستوى {مستوى_السنة}: {st.session_state.نقاط[مستوى_السنة]}")
                st.balloons()
            else:
...
# 6. منطق التنقل الرئيسي (الآن هو آمن)
